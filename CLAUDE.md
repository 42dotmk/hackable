...
| `hal/` | Hackable AI Layer: agentic loop + harness in C for any OpenAI-compatible model (tool calling, SSE), daemon `hald` + client `hal`, talks by voice through hstt and piper, conversations stored as mail (one Maildir per session under `~/.mail/hal/s/`; `hml send main@hal` starts a run, replies go to `user@hal`), skills as markdown, `make check` tests |
