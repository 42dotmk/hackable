...
| `hal/` | Hackable AI Layer: agentic loop + harness in C for any OpenAI-compatible model (tool calling, SSE), daemon `hald` + client `hal`, talks by voice through hstt and piper, conversations stored as mail (one Maildir per session under `~/.mail/hal/s/`; `hml send main@hal` starts a run, replies go to `user@hal`), child agents (`delegate` spawns a second hald with its own address and talks to it by mail), skills as markdown, `make check` tests |
...
- Only hed (`make test`), hterm (`make check`), hwm (`make check`, layout core only) and hal (`make check`) have test suites. The X11 projects are otherwise verified by running them (hwm and hweb can be driven under Xephyr; see their CLAUDE.md).
...
- `hal` and `hml` meet only on standard formats: hal writes mail through a sendmail-shaped command (`hml send -t` by default) and reads its inbox and its own sessions as plain Maildirs (format in `hal/MAIL.md`); hml's local delivery for `@hal` addresses is what makes that a message bus, and its index is how sessions are searched. Neither links or execs the other by name beyond that default.
